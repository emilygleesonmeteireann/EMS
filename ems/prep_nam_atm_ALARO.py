...
"""
Namelist for ALARO, atmospheric part
"""
...
def prep_nam_atm(ncfile, namin, timestep, namout='namarp', lsurfex=False):
    """
    Prepare ALARO namelist for MUSC simulation, 
    given information in filecase,
    and from ALARO namelist namin
    """

    logger.info('-'*40)
    logger.info('Prepare ALARO namelist for MUSC')
    ...
    #ensure ALARO physics
    nam['NAMPHY']['LA0MPS'] = ['.TRUE.']

    #Control moisture convergence computation for 3MT
    nam['NAMDYN']['NCOMP_CVGQ'] = ['2']

    #make qv gp, although we use it spectral in 3D 
    ...
    # DN: make list of variables specifically to allow runs with(out) prognostic graupel and with(out) 3MT
    varlist = ['L', 'I', 'R', 'S', 'TKE', 'TTE']
    if nam['NAMPHY']['L3MT'] == ['.TRUE.']:
        varlist.append('DAL')
        varlist.append('DOM')
        varlist.append('UAL')
        varlist.append('UOM')
        varlist.append('UNEBH')

    if nam['NAMPHY']['LGRAPRO'] == ['.TRUE.']:
        varlist.append('G')
    ...
    if attributes['radiation'] in ['off', 'tend']:
        nam['NAMPHY']['LRAY'] = ['.FALSE.']
        nam['NAMCFU']['LRAYS'] = ['.FALSE.']
        nam['NAMCFU']['LRAYD'] = ['.FALSE.']
    ...
    if attributes['surface_forcing_temp'] == 'surface_flux':
        if not(lsurfex):
            logger.warning('RTS_FORC not implemented, check your output PTS')
