...
"""
Namelist for AROME, atmospheric part
"""
...
def prep_nam_atm(ncfile, namin, timestep, namout='namarp', lsurfex=False):
    ...
    # Case with no shallow convection
    if attributes['shallow_convection'] in ['off']:
        nam['NAMPARAR']['LSHALLOWMF'] = ['.FALSE.']

    # Case with no turbulence
    if attributes['turbulence'] in ['off']:
        nam['NAMARPHY']['LTURB'] = ['.FALSE.']
